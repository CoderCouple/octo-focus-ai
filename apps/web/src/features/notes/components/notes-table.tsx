...
function matchesStatus(p: WorkspacePageSummary, filter: StatusFilter): boolean {
  if (filter === "all") return true;
  return noteStatusLabel(p.visibility).toLowerCase() === filter;
}
...
export function NotesTable({
  ...
  const filtered = useMemo(
    () => notes.filter((n) => matchesStatus(n, status)),
    [notes, status],
  );
  ...
  const counts = useMemo(
    () => ({
      all: notes.length,
      draft: notes.filter((n) => noteStatusLabel(n.visibility) === "Draft").length,
      published: notes.filter((n) => noteStatusLabel(n.visibility) === "Published").length,
    }),
    [notes],
  );

  const columns: ColumnDef<WorkspacePageSummary>[] = useMemo(
    () => [
      {
        accessorKey: "title",
        header: "Title",
      ...
      },
      {
        id: "status",
        header: "Status",
        accessorFn: (row) => noteStatusLabel(row.visibility),
        cell: ({ row }) => (
          <StatusBadge status={noteStatusLabel(row.original.visibility)} />
        ),
      },
      {
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-36">
                ...
                <DropdownMenuItem asChild>
                  <Link href={`/note/${row.original.id}`}>
                    Open
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuSeparator />
    ...
    >
      <div className="flex items-center justify-between gap-2 px-4 lg:px-6">
        ...
        <Select value={status} onValueChange={(v) => setStatus(v as StatusFilter)}>
          <SelectTrigger className="flex w-fit @4xl/main:hidden" size="sm" id="notes-status-selector">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
        ...
        <div className="flex items-center gap-2">
          ...
          <CreateProjectDialog workspaceId={workspaceId} label="New note" mode="notes" />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 px-4 lg:px-6">
        ...
        <div className="flex items-center gap-6">
          <div className="hidden items-center gap-2 lg:flex">
            <Label htmlFor="notes-rows-per-page" className="text-sm font-medium">
              Rows per page
            </Label>
            <Select
            ...
            >
              <SelectTrigger size="sm" className="w-20" id="notes-rows-per-page">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                rename.mutate(
                  { pageId: renameTarget.id, title: draft.trim() },
                  {
                    onSuccess: () => {
